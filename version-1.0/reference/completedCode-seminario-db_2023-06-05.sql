...
-- Tiempo de generación: 08-06-2023 a las 00:42:45
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `addGroup` (IN `Username` VARCHAR(50), IN `passwordd` VARCHAR(50))  BEGIN
    INSERT INTO `group` (`username`, `password`)
    VALUES (Username, passwordd);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `addGroupData` (IN `name` VARCHAR(50), IN `isActive` BOOLEAN, IN `amount` VARCHAR(50))  BEGIN
	INSERT INTO `groupdata` (`name`, `isActive`, `amount`)
    VALUES (name, isActive,amount);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `addUser` (IN `Username` VARCHAR(50), IN `passwordd` VARCHAR(50))  BEGIN
